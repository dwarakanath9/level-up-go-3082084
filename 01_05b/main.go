...
func matchSales(budget float64, items []SaleItem) []SaleItem {
	...
	for _, si := range items {
		if budget <= si.ReducedPrice {
			percentage := -(si.ReducedPrice - si.OriginalPrice/si.OriginalPrice*100)
			si.SalePercentage = percentage
			mi = append(mi, si)
		}
	}

	sort.Slice(mi, func(i, j int) bool {
		return int(mi[i].SalePercentage) > int(mi[j].SalePercentage)
	})
	return mi
}
